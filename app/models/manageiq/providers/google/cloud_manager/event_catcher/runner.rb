class ManageIQ::Providers::Google::CloudManager::EventCatcher::Runner < ManageIQ::Providers::BaseManager::EventCatcher::Runner
  def event_monitor_handle
    unless @event_monitor_handle
      access_key_id     = @ems.authentication_userid
      secret_access_key = @ems.authentication_password
      region            = @ems.provider_region
      queue_id              = @ems.guid
      @event_monitor_handle = ManageIQ::Providers::Google::CloudManager::EventCatcher::Stream.new(
        access_key_id, secret_access_key, region, queue_id)
    end
    @event_monitor_handle
  end

  def reset_event_monitor_handle
    @event_monitor_handle = nil
  end

  def stop_event_monitor
    @event_monitor_handle.try(:stop)
  ensure
    reset_event_monitor_handle
  end

  def monitor_events
    event_monitor_handle.start
    event_monitor_handle.each_batch do |events|
      _log.debug { "#{log_prefix} Received events #{events.collect(&:message)}" }
      @queue.enq events
      sleep_poll_normal
    end
  ensure
    reset_event_monitor_handle
  end

  def process_event(event)
    if filtered?(event)
      _log.info "#{log_prefix} Skipping filtered Google event [#{event["messageId"]}]"
    else
      _log.info "#{log_prefix} Caught event [#{event["messageId"]}]"
      EmsEvent.add_queue('add_google', @cfg[:ems_id], event)
    end
  end

  def filtered?(event)
    filtered_events.include?(event["messageType"])
  end
end
