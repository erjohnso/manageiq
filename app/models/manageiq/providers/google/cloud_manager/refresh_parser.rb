...
class ManageIQ::Providers::Google::CloudManager::RefreshParser < ManageIQ::Providers::CloudManager::RefreshParser
  extend Memoist
  ...
  def initialize(ems, options = nil)
    ...
    @deployment_manager = ems.deployment_manager
    @data               = {}
    @data_index         = {}
  ...
  def ems_inv_to_hashes
    ...
    $google_log.info("#{log_header}...")
    $google_log.info("#{log_header}...Complete")
  ...
  def get_flavors
    process_collection(ManageIQ::Providers::Google::InstanceTypes.all, :flavors) { |flavor| parse_flavor(flavor) }
  end

  def get_zones
    azs = @connection.zones
    process_collection(azs, :zones) { |az| parse_zone(az) }
  end
  ...
  def parse_zone(az)
    ...
    new_result = {
      :type    => ManageIQ::Providers::Google::CloudManager::one.name,
      :ems_ref => uid,
      :name    => name,
    }
  ...
  def self.key_pair_type
    ManageIQ::Providers::Google::CloudManager::AuthKeyPair.name
  end
  ...
  def parse_cloud_network(vpc)
    ...
    new_result = {
      ...
      :orchestration_stack => @data_index.fetch_path(:orchestration_stacks, vpc.tags["gcp:deploymentmanager:stack-id"]),
    }
    return uid, new_result
  end
  ...
  def parse_security_group(sg)
    ...
    new_result[:orchestration_stack] = @data_index.fetch_path(:orchestration_stacks, sg.tags["gcp:deploymentmanager:stack-id"])
  ...
  def parse_instance(instance)
    ...
    new_result = {
      ...
      :orchestration_stack => @data_index.fetch_path(:orchestration_stacks, instance.tags["gcp:deploymentmanager:stack-id"]),
    }
  ...
  def parse_floating_ip(ip)
    ...
    new_result = {
      :type               => ManageIQ::Providers::Google::CloudManager::FloatingIp.name,
      :ems_ref            => uid,
      :address            => address,
      :vm => associated_vm
    }
  ...
  def parse_stack(stack)
    ...
    new_result = {
      :type          => ManageIQ::Providers::Google::CloudManager::OrchestrationStack.name,
      :ems_ref       => uid,
      :name          => stack.name,
  ...
  def find_stack_resources(stack)
    # convert the GCP Resource Summary collection to an array to avoid the same API getting called twice
    ...
    resources = raw_resources.collect do |resource|
      ...
      child_stacks << physical_id if resource[:resource_type] == "GCP::DeploymentManager::Stack"
  ...
  def add_instance_disk(disks, size, name, location)
    super(disks, size, name, location, "google")
  end
  ...
  # TODO(erjohnso): enable memoize
  #memoize :get_flavors
  #memoize :get_zones
  #memoize :get_key_pairs
  #memoize :get_stacks
  #memoize :get_cloud_networks
  #memoize :get_private_images if @options["get_private_images"]
  #memoize :get_public_images  if @options["get_public_images"]
  #memoize :get_instances
  #memoize :get_floating_ips
